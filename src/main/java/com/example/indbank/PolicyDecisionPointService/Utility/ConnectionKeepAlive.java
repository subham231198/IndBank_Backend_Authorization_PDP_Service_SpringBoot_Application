...
public class ConnectionKeepAlive {
    ...
    @CircuitBreaker(name = "authenticationService", fallbackMethod = "keepAliveAuthenticationFallback")
    ...
    public void keepAliveAuthenticationFallback(Exception ex) {
        log.warn("Circuit Breaker OPEN - Authentication Service health check fallback: {}", ex.getMessage());
    }

    @CircuitBreaker(name = "health", fallbackMethod = "keepAliveAuthorizationFallback")
    @Scheduled(fixedDelay = 3000)
    ...
    public void keepAliveAuthorizationFallback(Exception ex) {
        log.warn("Circuit Breaker OPEN - Own Service health check fallback: {}", ex.getMessage());
    }
}